...
续办只处理此前未完成的批次：先核验原 Anki 笔记、本次遇见、单卡与音频，再抽样查询批次状态。抽样词仍在来源生词本时执行两个删除阶段；已移出来源生词本但仍在「全部生词」时只执行第二阶段；均已不存在时视为整批已清理。抽样状态不一致、读取失败或核验不通过时保留清单并停止新一轮导出。不比较时间或语境变化，不重新导入、重置卡片或同步。使用非默认 Anki 地址时，续办需传入原 `--anki-url`。

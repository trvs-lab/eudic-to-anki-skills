...
1. 仅当存在待生成音频时，先用同一 provider 和 voice 探测服务；全部单词已有有效 Anki 音频时直接复用，不调用 Edge-TTS。
...
已有 `[sound:...]` 必须先通过 AnkiConnect 读取并校验对应媒体；显式提供的有效本地 MP3 也可以复用。这属于输入复用，不是服务回退。任何新增或有变化的 note 都必须带有效音频，此规则不可通过命令参数关闭。

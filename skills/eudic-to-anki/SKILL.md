...
- Anchor and recall cards are separate templates/decks for learning phrase chunks, not isolated words. Anchor fronts show `target_chunk` plus `target_chunk_sentence`; anchor backs show `target_chunk_meaning` as a lightweight phrase confirmation, then optional source `example`, then a compact word header (`word` + IPA + word-level Chinese `meaning`). Recall card fronts show the cloze before the Chinese phrase meaning; recall backs show the left-aligned target chunk, optional source `example`, the same compact word header, and 1-2 collocations. There is no chunk-specific audio; both card types use the main word audio.
